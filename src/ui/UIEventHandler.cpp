 ...
 *
 * This function checks if the given button event corresponds to any parameter button mapping.
 * If a match is found, it updates the UI state to reflect whether the parameter button is held,
 * prints debug information to the serial output, and, if the button is pressed and not the "Note"
 * parameter, selects the AS5600 for the corresponding parameter.
 *
 * @param evt      The MatrixButtonEvent containing the button index and event type (pressed/released).
 * @param uiState  Reference to the UIState object to update the held state of parameter buttons.
...
static bool handleParameterButtonEvent(const MatrixButtonEvent &evt, UIState& uiState) {
    ...
    for (size_t i = 0; i < PARAM_BUTTON_MAPPINGS_SIZE; ++i) {
        ...
        if (evt.buttonIndex == mapping.buttonIndex) {
            ...
            // Automatically select AS5600 parameter if not the Note parameter and button is pressed
...
static bool handleStepButtonEvent(const MatrixButtonEvent &evt, UIState& uiState, Sequencer& seq1, Sequencer& seq2) {
    ...
    // --- Normal mode: differentiate between short/long press for edit/select/toggle ---
...
static void autoSelectAS5600Parameter(ParamId paramId, UIState& uiState) {
    ...
    bool isValid = true;
    switch (paramId) {
        case ParamId::Velocity: newAS5600Param = AS5600ParameterMode::Velocity; break;
        case ParamId::Filter:   newAS5600Param = AS5600ParameterMode::Filter;   break;
        case ParamId::Attack:   newAS5600Param = AS5600ParameterMode::Attack;   break;
        case ParamId::Decay:    newAS5600Param = AS5600ParameterMode::Decay;    break;
